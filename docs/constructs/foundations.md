...
The `sdlf-catalog{customSuffix}` Lambda function is used to populate `octagon-ObjectMetadata` automatically whenever a new object is uploaded or deleted from the `raw` and `stage` buckets. The `sdlf2-replicate` Lambda function copies the schema of any new Glue database or Glue database whose schema is updated.
